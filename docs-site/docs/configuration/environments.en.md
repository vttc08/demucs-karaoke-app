...
The main application loads values in this order:

1. Environment variables supplied by the process, Docker, or systemd.
2. Values from the local `.env` file.
3. Values saved in the application's runtime settings database.
4. Built-in application defaults.

An explicitly supplied environment variable takes precedence over a value saved from the Settings page after a restart. This is useful when a deployment must keep a path, service URL, executable, or other value fixed. Leave a variable unset when you want the Settings page and database value to control it.

Most boolean variables accept values such as `true`, `false`, `1`, `0`, `yes`, or `no`. Paths may be absolute or relative to the application directory unless noted otherwise. Byte limits use bytes, while timeouts and intervals use seconds.

The Docker Compose file supplies some application defaults directly through its `environment:` section. For a Linux or Windows installation, uncomment the corresponding variables in `.env` when you need to set them before startup.

## Main application

The following variables configure the main FastAPI application.
...
### Lyrics providers and languages
...
### Demucs and karaoke processing

These variables control how the main application connects to and requests work from the separate Demucs service. The service itself has additional variables documented below.
...
These variables configure the separate GPU or CPU processing service. Variables shared with the main application should normally use matching values.

### Service paths and access
...
### Main application connected to a remote Demucs service
...
### Docker Compose storage and logging
