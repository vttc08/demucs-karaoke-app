...
def test_runtime_settings_update_ytdlp_nightly_uses_channel_and_persists_uv_lock():
    """Nightly updates use yt-dlp's channel flag and prerelease-aware uv commands."""
    service = RuntimeSettingsService()
    pip_managed_error = subprocess.CalledProcessError(
        returncode=1,
        cmd=["/home/project/.venv/bin/yt-dlp", "--update-to", "nightly"],
        stderr="ERROR: You installed yt-dlp with pip or using the wheel from PyPi; Use that to update",
    )
    ...
    with patch.object(
        ...
        side_effect=[
            Mock(version="2026.06.09", binary_path="/home/project/.venv/bin/yt-dlp"),
            Mock(version="2026.07.04-nightly", binary_path="/home/project/.venv/bin/yt-dlp"),
        ],
    ):
        with patch("services.runtime_settings_service.shutil.which", return_value="/usr/bin/uv"):
            with patch("services.runtime_settings_service.subprocess.run") as mock_run:
                mock_run.side_effect = [
                    pip_managed_error,
                    Mock(stdout="Updated lockfile"),
                    Mock(stdout="Successfully installed yt-dlp-nightly"),
                ]
                result = service.update_ytdlp("nightly")

    assert result.updated is True
    assert mock_run.call_args_list[0].args[0][1:] == ["--update-to", "nightly"]
    ...
    assert mock_run.call_args_list[2].args[0][1:6] == [
        "pip",
        "install",
        "--upgrade",
        "--prerelease",
